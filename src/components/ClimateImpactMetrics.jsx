...
export default function ClimateImpactMetrics() {
      ...
      try {
        ...
        if (result && result.data && result.data.length > 0) {
          ...
          const processedMetrics = result.data.map((item, index) => {
            ...
            if (category.includes('carbon') || metricName.includes('carbon') || metricName.includes('co2')) {
              metricType = 'carbon';
            } else if (category.includes('water') || metricName.includes('water')) {
              metricType = 'water';
            } else if (category.includes('waste') || metricName.includes('waste') || metricName.includes('recycl')) {
              metricType = 'waste';
            } else if (category.includes('energy') || metricName.includes('energy') || metricName.includes('power')) {
              metricType = 'energy';
            } else if (category.includes('social') || metricName.includes('social') || metricName.includes('people') || metricName.includes('household')) {
              metricType = 'social';
            } else if (category.includes('renewable') || metricName.includes('renewable')) {
              metricType = 'energy';
            }
  ...
  useEffect(() => {
    ...
    metrics.forEach((metric, index) => {
      setTimeout(() => animateValue(metric.id, metric.targetValue), 200 + (index * 200));
    });
  }, [isVisible, metrics]);
  ...
  const formatValue = (metric, value) => {
    ...
    const unitPart = numericPart ? valueString.replace(numericPart, '').trim() : valueString;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden opacity-10">
        ...
        <div className="absolute bottom-10 left-10 w-80 h-80 bg-green-300 rounded-full filter blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
      </div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <p className="text-gray-600 text-lg max-w-3xl mx-auto leading-relaxed">
            Measuring our contribution to a sustainable future through verifiable impact metrics.
          </p>
        </div>
        ...
        {error && !loading && (
          <div className="bg-red-100 text-red-700 p-4 rounded-lg max-w-2xl mx-auto">
            <p className="font-medium">Error loading impact metrics:</p>
            <p>{error}</p>
          </div>
        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                      {metrics
                        ...
                        .map((metric) => (
                          ...
                          >
                            <div className={`${metric.bgColor} ${metric.borderColor} border-2 rounded-2xl p-6 h-full shadow-lg hover:shadow-2xl transition-all duration-500 relative overflow-hidden group-hover:border-opacity-60`}>
                              {/* Background pattern */}
                              ...
                              {/* Icon */}
                              <div className={`inline-flex items-center justify-center w-16 h-16 ${metric.bgColor.replace('50', '100')} rounded-xl mb-4 shadow-md transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}>
                                {metric.icon}
                              </div>
      ...
      {/* Floating elements */}
      <div className="absolute top-1/4 left-8 w-3 h-3 bg-green-400 rounded-full opacity-30 animate-bounce" style={{ animationDelay: '1s' }}></div>
      <div className="absolute top-3/4 right-8 w-4 h-4 bg-blue-400 rounded-full opacity-40 animate-bounce" style={{ animationDelay: '2s' }}></div>
      <div className="absolute top-1/2 left-1/4 w-2 h-2 bg-yellow-400 rounded-full opacity-50 animate-bounce" style={{ animationDelay: '1.5s' }}></div>
    </section>
  );
}