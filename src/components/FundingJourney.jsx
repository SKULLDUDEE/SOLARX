...
export default function FundingJourney({ companyId }) {
  ...
  useEffect(() => {
    const fetchFundingData = async () => {
      ...
      try {
        ...
        const apiUrl = `${API_URL || "http://localhost:1337/api"}/startups?populate[0]=funding&populate[1]=funding.investors&filters[id][$eq]=${companyId}`;
  ...
  >
    <div className="flex flex-col md:flex-row items-center justify-between md:justify-start mb-6 space-y-2 md:space-y-0 md:space-x-3 w-full">
      <div className="w-1/6 md:w-12 hidden md:block h-1.5 bg-gradient-to-r from-orange-500 to-red-500 rounded-full"></div>
  ...
  const FundingRound = ({ round, isVisible }) => (
    ...
    >
      <div
        className={`absolute left-0 w-7 h-7 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-125 ${
          round.isGrant
            ? "bg-blue-500"
            : round.isLoan
            ? "bg-green-500"
      ...
      <div className="bg-white p-4 sm:p-5 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-orange-200">
  ...
  const InvestorCard = ({ investor, index, isHovered, onMouseEnter, onMouseLeave, isVisible }) => (
    ...
    >
      <div
        className={`flex items-center p-3 sm:p-4 bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-orange-200 group cursor-pointer ${
          isHovered ? "scale-105 shadow-2xl ring-2 ring-orange-300" : ""
        }`}
      >
        <div
          className={`w-10 h-10 ${investor.color} rounded-full flex items-center justify-center text-white font-bold text-lg mr-3 shadow-md transition-all duration-300 group-hover:scale-110 flex-shrink-0`}
        >
          {investor.letter}
        </div>
    ...
    <section className="py-20 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      <div className="mx-auto px-4 sm:px-6 relative z-10">
        ...
        {!loading && !error && fundingRounds.length > 0 && (
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 max-w-screen-xl mx-auto">
            ...
            >
              ...
              <div className="space-y-6 relative">
                <div className="absolute left-3.5 top-0 bottom-0 w-0.5 bg-gradient-to-b from-orange-200 via-orange-300 to-orange-400 rounded-full"></div>
                {fundingRounds.map((round, index) => (
                  <FundingRound 
